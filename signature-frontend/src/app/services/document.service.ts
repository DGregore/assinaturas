...
export class DocumentService {
   ...
   * @param file O arquivo PDF a ser validado.
   * @returns Observable<{isValid: boolean, errors?: string[]}>
   */
  validatePdf(file: File): Observable<{isValid: boolean, errors?: string[]}> {
    const formData = new FormData();
    formData.append('file', file, file.name);
    return this.http.post<{isValid: boolean, errors?: string[]}>(`${this.apiUrl}/validate`, formData).pipe(
      catchError(this.handleError)
    );
  }

  /**
   * Uploads a new document file along with signatories data.
   * @param file The PDF file to upload.
   * @param signatories Array of signatories with their order.
   * @returns Observable<Document> The created document metadata.
   */
  uploadDocument(file: File, signatories: Partial<DocumentSignatory>[]): Observable<Document> {
    const formData = new FormData();
    formData.append('file', file, file.name);

    // O padrão para NestJS com DTOs aninhados é receber signatários como JSON
    formData.append('signatories', JSON.stringify(signatories));

    // Se precisar enviar description, adicione como argumento e aqui:
